...
def get_chatbot_response(
    ...
    try:
        ...
        # Detect out-of-scope topics with expanded categories
        out_of_scope_categories = {
            'finance': ['loan', 'credit', 'finance', 'money', 'invest', 'stock', 'bitcoin', 'crypto', 'mortgage', 'debt'],
            'general_medical': ['cancer', 'surgery', 'tumor', 'allergy', 'fracture', 'pregnancy', 'birth', 'flu', 'covid', 'vaccine'],
            'tech': ['computer', 'laptop', 'phone', 'wifi', 'internet', 'software', 'hardware', 'coding', 'program'],
            'personal': ['dating', 'relationship', 'marriage', 'divorce', 'therapy', 'counseling'],
            'legal': ['lawsuit', 'attorney', 'lawyer', 'legal', 'court', 'judge', 'sue'],
            'other_services': ['restaurant', 'hotel', 'flight', 'booking', 'reservation']
        }
        
        # Check if query contains any out-of-scope keywords
        query_lower = user_query.lower()
        
        for category, keywords in out_of_scope_categories.items():
            if any(keyword in query_lower for keyword in keywords):
                return (
                    f"I understand you're asking about {category.replace('_', ' ')}, but I'm specifically designed to help with "
                    f"stroke risk assessment, prevention, and related information. My expertise is limited to topics such as:\n\n"
                    f"• Stroke risk factors and prevention strategies\n"
                    f"• Understanding your stroke risk prediction results\n"
                    f"• General information about stroke symptoms and warning signs\n"
                    f"• Lifestyle modifications that may reduce stroke risk\n\n"
                    f"Is there something about stroke risk or your prediction results I can help you with instead?"
                )
        
        # Get response from Gemini for in-scope queries
...
def get_fallback_response(prompt: str, user_data: Optional[Dict[str, Any]] = None, prediction: Optional[float] = None) -> str:
    ...
    if any(word in prompt_lower for word in ["risk", "stroke", "chance", "probability", "likelihood"]):
        ...
        return (f"I understand you're asking about something that may not be directly related to stroke risk. "
                f"As a specialized assistant, I can best help with stroke risk factors, prevention, symptoms, "
                f"treatment options, and interpreting your prediction results from our model.{disclaimer}")